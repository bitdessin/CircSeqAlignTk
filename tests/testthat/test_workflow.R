...
test_that('workflow with Bowtie2', {
    ...
    # index
    ref_index <- build_index(fasta = fa, output = 'tmp_bt2index')
    # run with default arguments
    aln <- align_reads(fastq = fq, index = ref_index, output = 'tmp_bt2output')
...
test_that('workflow with HISAT2', {
    ...
    fq <- system.file(package='CircSeqAlignTk', 'extdata', 'srna.fq.gz')
    # index
    ref_index <- build_index(fasta = fa,
                             output = 'tmp_ht2index', aligner = 'hisat2')
    # run with default arguments
    aln <- align_reads(fastq = fq, index = ref_index, output = 'tmp_ht2output',
                       aligner = 'hisat2')
    # run with additional arguments
    aln <- align_reads(fastq = fq, index = ref_index, output = 'tmp_ht2output',
                       aligner = 'hisat2',
...
test_that('workflow - no reads aligned on edges', {
    ...
    fa <- system.file(package='CircSeqAlignTk', 'extdata', 'FR851463.fa')
    ref_index <- build_index(fasta = fa, output = 'tmp_index')
    aln <- align_reads(fastq = 'tmp_sim_reads_1.fq.gz',
...
test_that('workflow - reads aligned on edges', {
    ...
    fa <- system.file(package='CircSeqAlignTk', 'extdata', 'FR851463.fa')
    ref_index <- build_index(fasta = fa, output = 'tmp_index')
    aln <- align_reads(fastq = 'tmp_sim_reads_2.fq.gz',
...
test_that('workflow - reads without adapter and mismatches correctly aligned', {
    ...
    ref_index <- build_index(fasta = viroid_seq, output = 'tmp_index')
