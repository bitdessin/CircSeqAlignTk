...
#'
#' @param x A \code{\link{CircSeqAlignTk-class}} object generated by
...
#'                    read_lengths = seq(21, 24))
#' }
#' alncov <- calc_coverage(aln)
#' @seealso \code{\link{CircSeqAlignTk-class}},
#'     \code{\link{CircSeqAlignTkCoverage-class}}, \code{\link{align_reads}}
#' @export
calc_coverage <- function(aln) {
    depth_mat_1 <- .calc_coverage(aln@clean_bam[1], aln@reference@length)
    depth_mat_2 <- .calc_coverage(aln@clean_bam[2], aln@reference@length,
                                  aln@reference@cut_loc)
    ...
    mat_fwd <- matrix(0, ncol = length(read_len), nrow = aln@reference@length)
    mat_rev <- matrix(0, ncol = length(read_len), nrow = aln@reference@length)
...
#' aligned in the forward and reversed strands, respectively.
#'
